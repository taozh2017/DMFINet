...
class CrossAttentionBlock(nn.Module):
    def __init__(self, img_size=64, img_size_y=32, patch_size=4, in_chans=128, num_classes=1000, embed_dim=2048, depth=3,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, patch_first=False, un_patch=False, patch_first_y=False, un_patch_y=False):
        ...
        self.patch_first_y = patch_first_y
        ...
        self.un_patch_y = un_patch_y
        if self.un_patch_y:
            self.un_patch_embed_y = PatchUnembed(img_size=img_size_y, patch_size=patch_size, in_chans=in_chans,
                                               embed_dim=embed_dim)
    ...
    def forward(self, x, y, pad_mask=None):
        if self.patch_first:
            x = self.patch_embed(x)  # 后续尝试使用view方法
            x = x + self.pos_embed
            x = self.pos_drop(x)
        if self.patch_first_y:
            y = self.patch_embed_y(y)  # 后续尝试使用view方法
        ...
        re_y = self.blk_y(y, x)
        if self.un_patch:
            re_x = self.un_patch_embed(re_x)
        if self.un_patch_y:
            re_y = self.un_patch_embed_y(re_y)

        return re_x, re_y

if __name__ == "__main__":
    cat = CrossAttentionBlock(img_size=64, img_size_y=32, patch_size=4, in_chans=128, embed_dim=1024, depth=3, num_heads=8,
                    mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                     drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, patch_first=True, patch_first_y=True)
    cat1 = CrossAttentionBlock(img_size=32, img_size_y=64, patch_size=4, in_chans=128, embed_dim=1024, depth=3, num_heads=8,
                    mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                     drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, un_patch=True, un_patch_y=True)
