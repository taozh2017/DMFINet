...
class CycleGANModel_2D(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_A = networks.define_G(opt.patch_size, opt.mask_ratio, opt.base_filter, opt.which_model_netG, opt.norm,
                                        self.gpu_ids, opt.res_net, opt.pretrained_path, opt.res_num, opt.large_num)
        ...
        if self.isTrain:
            ...
            print("dis: ", opt.which_model_netD)
    ...
    def backward_G(self):
        ...
        # flops, params = profile(self.netG_A, (self.real_A, self.A_down_image, self.A_re_down_image,))
        # print('flops: ', flops, 'params: ', params)
        # print('flops: %.2f M, params: %.2f M' % (flops / 1000000.0, params / 1000000.0))

        mask_image, A_re_result, self.fake_B, self.vis = self.netG_A(self.real_A, self.A_down_image, self.A_re_down_image)
        pred_fake = self.netD_A.forward(self.fake_B)
        self.loss_G_A = self.criterionGAN(pred_fake, True)
        # D_B(G_B(B))
        mask_image, B_re_result, self.fake_A,_ = self.netG_B(self.real_B, self.B_down_image, self.B_re_down_image)
        pred_fake = self.netD_B.forward(self.fake_A)
        self.loss_G_B = self.criterionGAN(pred_fake, True)
        # Forward cycle loss
        self.fake_B_down_image, self.fake_B_re_down_image = getDownAndRe(self.fake_B, re_down=self.opt.re_down, re_augment=False)
        # print(self.fake_B_re_down_image.shape)
        mask_image, cyc_B_re_result, self.rec_A,_ = self.netG_B(self.fake_B, self.fake_B_down_image, self.fake_B_re_down_image)
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * lambda_A
        # Backward cycle loss
        self.fake_A_down_image, self.fake_A_re_down_image = getDownAndRe(self.fake_A, re_down=self.opt.re_down, re_augment=False)
        mask_image, cyc_A_re_result, self.rec_B,_ = self.netG_A(self.fake_A, self.fake_A_down_image, self.fake_A_re_down_image)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.A_mask_image, self.A_re_result, self.fake_B, self.vis = self.netG_A(self.real_A, self.A_down_image, self.A_re_down_image)
            mask_image, re_result, self.fake_A, self.vis_reverse = self.netG_B(self.real_B, self.B_down_image, self.B_re_down_image)

    def saveImages(self, result_dir, i):
        origin_path = result_dir + "input/"
        output_path = result_dir + "output/"
        gt_path = result_dir + "gt/"
        reverse = result_dir + "reverse/" 
        ...
        fake_A = torch.rot90(fake_A, k=1, dims=(2, 3))
        fake_A = torch.flip(fake_A, [3])
        ...
        save_image(fake_A, reverse + str(index) + 'reverse.png', normalize=False)
    ...
    def rcalculateMetrics(self):
        pred, gt = self.fake_A.cpu().detach().numpy().squeeze(), self.real_A.cpu().detach().numpy().squeeze()  # [-1 1]
        pred, gt = (pred + 1) / 2, (gt + 1) / 2  # to 0-1
        a = ssim(pred, gt, data_range=1)
        b = psnr(pred, gt)
        c = nmse(pred, gt)
    ...
    def saveImagesHan(self, result_dir, i):
        ...
        # save_tensor_to_niigz(result_dir, fake_B, 9999, "nii.gz")
    ...
    def getData(self):
        A = self.real_A
        A_down = self.A_down_image
        A_redown_mask = self.A_mask_image
        fake_B = self.fake_B
        re_result = self.A_re_result

       # 对所有张量执行 +1 / 2 操作
        A = (A + 1) / 2.
        A_down = (A_down + 1) / 2.
        A_redown_mask = (A_redown_mask + 1) / 2.
        fake_B = (fake_B + 1) / 2.
        re_result = (re_result + 1) / 2.

        A = torch.rot90(A, k=1, dims=(2, 3))
        A = torch.flip(A, [3])
        A_down = torch.rot90(A_down, k=1, dims=(2, 3))
        A_down = torch.flip(A_down, [3])
        A_redown_mask = torch.rot90(A_redown_mask, k=1, dims=(2, 3))
        A_redown_mask = torch.flip(A_redown_mask, [3])
        re_result = torch.rot90(re_result, k=1, dims=(2, 3))
        re_result = torch.flip(re_result, [3])
        fake_B = torch.rot90(fake_B, k=1, dims=(2, 3))
        fake_B = torch.flip(fake_B, [3])

        save_image(A,  "model/A.png", normalize=False)
        save_image(A_down, 'model/A_down.png', normalize=False)
        save_image(A_redown_mask, 'model/A_redown_mask.png', normalize=False)
        save_image(fake_B, 'model/fake_B.png', normalize=False)
        save_image(re_result, 'model/re_result.png', normalize=False)
    ...
    #     for i, tensor in enumerate(self.vis):  
    ...
    #             save_image(tensor_a,  "model/" + str(i*128+j) + ".png", normalize=False)
    ...
    def getMSFF(self):
        ...
        if not os.path.exists(output_folder):  
            os.makedirs(output_folder)  
        tensor = tensor.squeeze()   
        save_path = "model/visHot.png"
        tensor = (tensor - tensor.min()) / (tensor.max() - tensor.min())  
        # 计算均值特征图
        mean_feature_map = torch.mean(tensor, dim=0, keepdim=True)
        # 归一化处理
        # mean_feature_map = torch.clamp(mean_feature_map, min=0, max=0.2) 
        mean_feature_map = (mean_feature_map - mean_feature_map.min()) / (mean_feature_map.max() - mean_feature_map.min())  
        save_image(mean_feature_map,  "model/vis.png", normalize=False)
        mean_feature_map = torch.clamp(mean_feature_map, min=0, max=0.1)  # 根据需要调整 min 和 max

        # 保存为 PNG 图像（原始张量）
        # save_image(mean_feature_map,  "model/vis.png", normalize=True)

        # 转换为 NumPy 数组
        mean_feature_map = mean_feature_map.squeeze().cpu().numpy()

        # 使用 matplotlib 绘制热力图，调整 vmin 和 vmax 增强对比度
        import matplotlib.pyplot as plt

        # 假设 mean_feature_map 是已经归一化并转换为 NumPy 数组的张量
        plt.imshow(mean_feature_map, cmap='hot', interpolation='nearest', vmin=0, vmax=0.08)

        # 移除标题和坐标轴等无关元素
        plt.axis('off')  # 关闭坐标轴
        plt.savefig('model/visHot.png', bbox_inches='tight', pad_inches=0)  # 保存并移除边框填充
        plt.close()  # 关闭图形以避免显示



def save_tensor_to_niigz(result_dir, tensor, number, name):
    os.makedirs(result_dir, exist_ok=True)
    save = tensor.cpu().numpy().squeeze()
    save = np.expand_dims(save, axis=1)  # 512 1 512
    save = (save + 1)/2  # -1 1 to 0 1
    affine = [[-0., - 0., - 0.79999995, 80.88793182], [-0.80000001, - 0., 0., 147.58079529],
            [0., - 0.80000001, - 0., 120.28206635],
            [0., 0., 0., 1.]]  # 通过将文件夹下面的文件转为nii.gz文件，然后通过nibalel获取，得到affine属性
    img = nib.Nifti1Image(save, affine=affine)
    # 保存为Nii文件
    nii_filename = result_dir+str(number) + str(name) + ".nii"
    nib.save(img, nii_filename)
    print(str(number) +'.niigz')
