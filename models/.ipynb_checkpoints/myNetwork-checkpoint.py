...
# from .Attention.MultiScaleCrossAttention import CrossAttentionBlock
# from .Attention.pretrain import transformer_configs
# from .Attention.pretrain.LoadParameter import block
# from .Attention.MultiScaleCrossAttentionMutual import PatchUnembed
...
class UpLayer(nn.Module):
    def __init__(self, in_cha, out_cha, res_net=False, norm_layer=nn.BatchNorm2d, use_bias=False, padding_mode="reflect"):
        ...
        self.layer = LUConv(out_cha, out_cha, "relu", norm_layer, use_bias, padding_mode, kernel_size=3, padding=1)
...
class MyModel(nn.Module):
    def __init__(self, act='relu', base_filter=64, bottleneck_layers=6, re_outcha=1,
        ...
        # 第二次底层，尝试交互
        self.re_bottleNeckFirst = _make_nConv(self.base_filter * 2, self.bottle_neck_channel, act, res_net=res_net,
                                            norm_layer=norm_layer, use_bias=use_bias, padding_mode=self.padding_mode)
        self.re_bottleNeckLast = _make_nConv(self.bottle_neck_channel, self.base_filter * 2, act, res_net=res_net,
                                              norm_layer=norm_layer, use_bias=use_bias, padding_mode=self.padding_mode)
        ...
        # 第二次底层，尝试交互
        self.main_bottleNeckFirst = _make_nConv(self.base_filter * 2, self.bottle_neck_channel, act, res_net=res_net,
                                              norm_layer=norm_layer, use_bias=use_bias, padding_mode=self.padding_mode)
        self.main_bottleNeckLast = _make_nConv(self.bottle_neck_channel, self.base_filter * 2, act,
                                                res_net=res_net, norm_layer=norm_layer, use_bias=use_bias,
                                                padding_mode=self.padding_mode)

        # TWO  相互使用CAT
        # self.bottle_fusionOne_main = CrossAttention(self.bottle_neck_channel, self.att_dim)
        # self.bottle_fusionTwo_main = CrossAttention(self.bottle_neck_channel, self.att_dim)
        # self.bottle_fusionThree_main = CrossAttention(self.bottle_neck_channel, self.att_dim)
        # self.bottle_fusionOne_re = CrossAttention(self.bottle_neck_channel, self.att_dim)
        # self.bottle_fusionTwo_re = CrossAttention(self.bottle_neck_channel, self.att_dim)
        # self.bottle_fusionThree_re = CrossAttention(self.bottle_neck_channel, self.att_dim)

        # self.mainFusionOne = mainFusion(self.base_filter * 4, self.base_filter * 2, res_net=res_net,
        #                                 norm_layer=norm_layer, use_bias=use_bias, padding_mode=self.padding_mode)

        # self.bottle_fusionOne_main = CBAM(in_planes=self.bottle_neck_channel, ratio=8, kernel_size=7)
        # self.bottle_fusionOne_main = LSKblock(self.bottle_neck_channel)
        self.bottle_fusionOne_main = LargeConvAdd(self.bottle_neck_channel)
        ...
        # MaskGan 添加code
#         self.deconv_attention = nn.Conv2d(self.base_filter, self.n_att, 1, 1, 0)
#         self.deconv_content = nn.Sequential(nn.ReflectionPad2d(3),
#                                        nn.Conv2d(self.base_filter * 1, re_outcha * self.n_att, kernel_size=7, padding=0),
#                                        nn.Tanh())
    ...
    def forward(self, x, down_image, re_down_image):
        ...
        # TWO
        self.main_bottle = self.main_bottleNeckFirst(self.out_two)
        self.main_bottle = self.main_bottlneck_model(self.main_bottle)
        self.re_bottle = self.re_bottleNeckFirst(re_two)
        ...
        main_up_two = self.mainFusionTwo(self.skip_outtwo, up_skiptwo, main_up)
        main_up_three = self.mainFusionThree(self.skip_outone, up_skipone, main_up_two)

        main_result = self.main_last(main_up_three)

#         # MaskGan处理方法
#         main_content = self.deconv_content(main_up_three)
#         images = []
#         for i in range(self.n_att-1):
#             images.append(main_content[:, i*self.output_nc : (i+1)*self.output_nc, :, :])

#         main_attention = self.deconv_attention(main_up_three)
#         softmax_ = torch.nn.Softmax(dim=1)
#         attention = softmax_(main_attention)
#         attentions = []
#         for i in range(self.n_att):
#             attentions.append(attention[:, i:(i+1), :, :])

#         # output = torch.zeros_like(input, dtype=torch.float32)  # 初始化output为float32类型
#         output = torch.zeros_like(x)
#         for i in range(self.n_att):
#             if i < self.n_att - 1:
#                 # out = images[i].to(torch.float32) * attentions[i].to(torch.float32)
...
#             output += out
        ...
        # return main_result, main_result, main_result
