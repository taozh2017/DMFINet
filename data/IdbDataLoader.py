...
def upsample_image(img, size=162):
    # Convert NumPy array to PyTorch tensor
    img = torch.tensor(img, dtype=torch.float).unsqueeze(0).unsqueeze(0)  # Assuming img is a 2D array (H, W)
...
class IdbDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        npy_one = np.load(self.trainA_list[index])
        npy_two = np.load(self.trainB_list[index])
        ...
        # 添加MaSKgAN的方法
        img_A, mask_A = get_3d_mask(img, min_=0, max_=1, th=15, width=10) # t1
        gt_B, mask_B = get_3d_mask(img, min_=0, max_=1, th=40, width=5)  #ct
...
def get_loader(batchsize, shuffle, pin_memory=True, source_modal='t1', target_modal='ct',
               img_size=162, img_root='data/', mode="train/", data_rate=1, num_workers=8, sort=False, argument=False,
               random=False, re_down=True):
    dataset = IdbDataset(source_modal=source_modal, target_modal=target_modal,
...
if __name__ == '__main__':
    dataloader = get_loader(batchsize=1, shuffle=True, pin_memory=True, source_modal='t1',
                             target_modal='ct', img_size=160, num_workers=8,
                             img_root='../idbData/', mode="train/", data_rate=1, argument=True, random=False, re_down=True)
